...
namespace Graphdotnetv4.Users.MailFolders.Messages {
    ...
    public class MessagesRequestBuilder {
        ...
        /// <param name="h">Request headers</param>
        /// <typeparam name="T">The generic type for the model, must inherit Message</typeparam>
        /// </summary>
        public RequestInfo CreatePostRequestInfo<T>(T body, Action<IDictionary<string, string>> h = default) where T : Message, IParsable<T>, new(){
            var requestInfo = new RequestInfo {
        ...
        /// <typeparam name="T">The generic type for the model, must inherit MessagesResponse</typeparam>
        /// </summary>
        public async Task<T> GetAsync<T>(Action<GetQueryParameters> q = default, Action<IDictionary<string, string>> h = default, IResponseHandler responseHandler = default) where T : MessagesResponse, IParsable<T>, new(){
        ...
        /// <typeparam name="T">The generic type for the model, must inherit Message</typeparam>
        /// </summary>
        public async Task<T> PostAsync<T>(T body, Action<IDictionary<string, string>> h = default, IResponseHandler responseHandler = default) where T : Message, IParsable<T>, new(){
